...
- **展开面板 = 液态悬浮面板**：通过 portal 渲染到 `<body>`，`position: fixed` 锚定在胶囊下方（右对齐），**不被会话头部/滚动容器裁剪**，随滚动/缩放重新锚定；`z-index 1000` 悬浮于一切内容之上。**液态质感**：半透明玻璃表面（`backdrop-filter` 模糊）+ 轻微不规则的液滴圆角 + 顶层内高光；**420ms 液滴沉降入场**（圆角/模糊随沉降形变），之后进入**每 5.5s 一次的微浮动**；收起时**液滴回吸**（230ms 下坠、圆角张开、模糊加深）。
- **生命周期**：**任务进行时胶囊自动展开**面板（`autoExpandRunning`，默认开；手动收起后本次任务不再强制打开）；**计划全部完成（或空计划）后面板 1.5s 自动缩回紧凑胶囊态**，完成态胶囊保留（时长可配）；失败/中止不自动缩回——错误入口保持可见（可配置失败自动展开）。支持 `prefers-reduced-motion`，无烟花/Toast/弹窗。
