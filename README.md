...
- **展开面板 = 悬浮面板**：通过 portal 渲染到 `<body>`，`position: fixed` 锚定在胶囊下方（右对齐），**不被会话头部/滚动容器裁剪**，随滚动/缩放重新锚定；`z-index 1000` 悬浮于一切内容之上。滚动时面板始终跟着胶囊走，绝不"脱钩"。
- **生命周期**：**任务进行时胶囊自动展开**面板（`autoExpandRunning`，默认开；手动收起后本次任务不再强制打开）；**计划全部完成（或空计划）后面板 1.5s 自动缩回紧凑胶囊态**，完成态胶囊保留（时长可配）；失败/中止不自动缩回——错误入口保持可见（可配置失败自动展开）。展开是 240ms 的弹性 grow（morph，带轻微过冲），收起是对称的 150ms 淡出回落，无烟花/Toast/弹窗。
