...
- **生命周期**：**任务进行时胶囊自动展开**面板（`autoExpandRunning`，默认开；手动收起后本次任务不再强制打开）；**计划全部完成（或空计划）后面板 1.5s 自动缩回紧凑胶囊态**，完成态胶囊保留（时长可配）；失败/中止不自动缩回——错误入口保持可见（可配置失败自动展开）。展开是 200ms 的弹性 grow（morph），收起是对称的 150ms 淡出回落，无烟花/Toast/弹窗。
