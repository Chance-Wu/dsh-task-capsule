...
export const zh = {
  ...
  'panel.retry': '重试',
  'panel.open': '打开',
  'panel.retryPrompt': '继续',
  'panel.subagents': '子代理 {count} · 已完成{done} 进行中{active} 待处理{pending} · {files} 文件',
  'panel.subagentRow': '{done}/{total} · {files} 文件',
  'tree.completed': '已完成 {count}',
  'settings.loading': '加载中…',
  'settings.title': '任务胶囊',
  'settings.groupDisplay': '显示',
  'settings.groupBehavior': '行为',
  'settings.groupAppearance': '外观',
  'settings.groupDebug': '调试',
...
export const en: Record<TaskCapsuleKey, string> = {
  ...
  'chip.turns': '{count} turns',
  'history.retried': 'Retried {count}×',
  ...
  'panel.retry': 'Retry',
  'panel.open': 'Open',
  'panel.retryPrompt': 'Continue',
  'panel.subagents': 'Subagents {count} · {done} done · {active} active · {pending} pending · {files} files',
  'panel.subagentRow': '{done}/{total} · {files} files',
  'tree.completed': 'Completed {count}',
